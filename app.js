...
// ... All helper functions (getTodayStr, setSaved, debounce, etc.) remain the same ...
...
function renderHabitRibbon(habitStates = {}) {
  ...
  HABITS.forEach(h => {
    ...
    pill.dataset.id = h.id; pill.title = h.text; 
    pill.innerHTML = `<span class="dot"></span><span class="label">${h.shortLabel}</span>`;
    pill.addEventListener('click', () => {
      const newState = !habitStates[h.id]; habitStates[h.id] = newState;
...
function checkAutoHabits(entryData) { if (!entryData) return; entryData.habits = entryData.habits || {}; const filledPriorities = (entryData.priorities || []).filter(p => p && p.trim() !== '').length; if (filledPriorities >= 3) { entryData.habits.setPriorities = true; } const filledGratitudes = (entryData.gratitude || []).filter(g => g && g.trim() !== '').length; if (filledGratitudes >= 3) { entryData.habits.gratitude = true; } }
async function loadEntry(dateStr) {
  if(!dateStr) return; dom.journal.value = 'Loading...';
  ...
  if(data) {
    ...
    checkAutoHabits(data); renderHabitRibbon(data.habits || {});
    ...
    dom.journal.value = JOURNAL_TEMPLATE; renderList('priorities-list', []); renderList('gratitude-list', []); renderHabitRibbon({});
    if ($('deep-work-log')) $('deep-work-log').value = ''; if ($('food-log-entry')) $('food-log-entry').value = '';
  }
...
function renderList(containerId, items = []) {
  ...
  for(let i=0;i<Math.max(3, items.length);i++){
    ...
    wrap.appendChild(txt); autosize(txt); 
  }
}
function gatherEntry() { const habits = {}; document.querySelectorAll('.habit-pill').forEach(p => { habits[p.dataset.id] = p.classList.contains('done'); }); return { content: dom.journal.value, foodLog: $('food-log-entry')?.value || '', deepWork: $('deep-work-log')?.value || '', priorities: Array.from(document.querySelectorAll('#priorities-list textarea')).map(t => t.value), gratitude: Array.from(document.querySelectorAll('#gratitude-list textarea')).map(t => t.value), habits }; }
function triggerAutosave(){
  setSaved(false);
  debounce(async () => {
    const dateStr = dom.dateInput.value, entryData = gatherEntry();
    checkAutoHabits(entryData); saveLocalDraft(dateStr, entryData);
    try { await saveEntryToFirestore(dateStr, entryData); setSaved(true); renderHabitRibbon(entryData.habits); } catch(err){ console.error('save failed', err); }
  }, 900);
}
...
async function handleHabitGridChange(e) {
    ...
    try { let entryData = await loadEntryFromFirestore(dateStr) || { habits: {} }; if (!entryData.habits) entryData.habits = {}; entryData.habits[habitId] = e.target.checked; await saveEntryToFirestore(dateStr, { habits: entryData.habits });
    } catch (err) { console.error("Failed to update habit from grid:", err); e.target.checked = !e.target.checked; alert("Failed to save habit.");
    } finally { e.target.disabled = false; }
}

// ---------- Habit Tracker Grid Rendering (MODIFIED) ----------
function renderHabitTrackerGrid(year, month, monthlyData = {}, habitCounts = {}) {
    dom.habitTrackerContainer.innerHTML = '';
    const daysInMonth = new Date(year, month + 1, 0).getDate();

    const grid = document.createElement('div');
    grid.className = 'habit-tracker-grid';
    // Widen the first column to make space for the count
    grid.style.gridTemplateColumns = `160px repeat(${daysInMonth}, minmax(35px, 1fr))`;
    ...
    HABITS.forEach(habit => {
        ...
        // Get the count for this habit
        const count = habitCounts[habit.id] || 0;
        habitLabel.innerHTML = `<span>${habit.shortLabel}</span> <span class="habit-count">${count}</span>`;
...
function renderCalendar() {
  ...
  Promise.all(promises).then(arr => {
    ...
    // --- CALCULATE HABIT COUNTS FOR THE MONTH ---
    const habitCounts = {};
    ...
    arr.forEach(obj => {
      ...
      // Tally up habit completions
      if(obj.data?.habits){
        HABITS.forEach(h => {
          if(obj.data.habits[h.id]) {
            habitCounts[h.id]++;
          }
        });

        // Update calendar day visual
    ...
    // Pass the calculated counts to the grid renderer
    renderHabitTrackerGrid(year, month, monthlyData, habitCounts);
  });
}

// ... Small UI helpers (toggleOptional, addListRow, showPane) remain the same ...
function toggleOptional(key, show) { const opt = Array.from(document.querySelectorAll('.optional')).find(o => o.dataset.key === key); if(!opt) return; const body = opt.querySelector('.optional-body'); const isHidden = body.classList.contains('hidden'); if(show === undefined) show = isHidden; body.classList.toggle('hidden', !show); if (show) { body.querySelectorAll('textarea').forEach(autosize); } }
function addListRow(containerId, value='') { const wrap = document.getElementById(containerId); const txt = document.createElement('textarea'); txt.className = 'small-input'; txt.rows = 1; txt.value = value; txt.addEventListener('input', () => { autosize(txt); triggerAutosave(); }); wrap.appendChild(txt); txt.focus(); }
function showPane(name) { dom.tabDaily.classList.toggle('active', name==='daily'); dom.tabCalendar.classList.toggle('active', name==='calendar'); dom.calendarPane.classList.toggle('hidden', name!=='calendar'); document.querySelector('#left-col').classList.toggle('hidden', name==='calendar'); if (dom.mbDaily) { dom.mbDaily.classList.toggle('active', name === 'daily'); dom.mbCalendar.classList.toggle('active', name === 'calendar'); } if (name !== 'calendar' && dom.habitTrackerContainer) { dom.habitTrackerContainer.innerHTML = ''; } }
...
function initializeAppLogic() {
  ...
  setSaved(false);
  renderHabitRibbon({});
  // updateMonthlyStatsFromUI() call is removed
}

// ----- MONTHLY STATS FUNCTION IS ENTIRELY REMOVED -----

// ... Boot & Login logic remains the same ...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    const attemptLogin = () => {
        if (passwordInput.value === 'Swag2') {
            loginOverlay.style.display = 'none'; appRoot.classList.remove('hidden'); siteHeader.classList.remove('hidden');
            initializeAppLogic();
        } else {
            loginBox.classList.add('error-shake'); passwordInput.value = '';
